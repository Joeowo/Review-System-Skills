...
class QuestionRepository:
    ...
    def get_wrong_questions(self, user_id: str = "default", for_practice: bool = True) -> List[Question]:
        ...
        # 优化选题算法：
        # 1. 优先选择最近答错的（consecutive_correct=0）
        # 2. 次优选择答对过但未掌握的（consecutive_correct=1）
        # 3. 已掌握但保留的题目最后
        # 4. 在同优先级内，按错误率降序排列（错误率高的优先）
        wrong_questions.sort(
            key=lambda q: (
                q.mastered_but_keep,  # 已掌握的排最后
                q.consecutive_correct,  # 连续正确次数越少越优先
                -(1 - q.times_correct / max(q.times_presented, 1))  # 错误率越高越优先（负号降序）
            )
        )

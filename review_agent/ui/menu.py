...
[bold cyan]0.[/bold cyan] [white]退出[/white]              [dim]退出系统[/dim]
    ...
    def _start_quiz(self):
        ...
        while True:
            # 使用SM-2调度获取题目
            due_questions = self.scheduler.get_due_questions(all_questions, limit=5)

            if not due_questions:
                # 没有到期题目，获取新题
                new_questions = self.scheduler.get_new_questions(all_questions, limit=5)
            ...
            # 开始一轮刷题
            self.console.print(f"[bold cyan]═══ 第 {round_num} 轮 ═══[/bold cyan]")
            user_records = self.quiz_interface.start_round(due_questions, round_num)

            # 保存答题记录和更新题目统计
            for record in user_records:
                self.stats_repo.save_answer(record)

            # 保存更新后的题目（due_questions 中的对象已被修改）
            # 按会话分组，避免重复加载
            all_questions_dict = self.question_repo.load_all()
            session_updates = {}  # session_id -> {question_id: updated_question}

            for answered_q in due_questions:
                # 找到该题目所属的会话
